#!/usr/bin/env python3
"""
Database Import Script for Article Editor Case Studies
...
def import_case_study(conn, file_path: Path) -> bool:
    ...
    try:
        ...
        # Validate agent_slug
        if data.get('agent_slug') != 'article-editor':
            print(f"  ✗ Invalid agent_slug: Expected 'article-editor', got '{data.get('agent_slug')}'")
            return False
...
def main():
    ...
    print("=" * 70)
    print("ARTICLE EDITOR CASE STUDY IMPORT")
    print("=" * 70)
